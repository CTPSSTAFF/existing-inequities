...
access_layer <- st_crop(access_layer, boundary)
```
```{r warnings=F, message=FALSE}
ggplot()+
  geom_stars(data=access_layer, alpha = .7)+
...
workers <- read_stars("../data/lodes-data-2018 Workers total conveyal.tif")%>% 
  st_transform(3857)
workers_crop <- st_crop(workers, boundary)
# note significant difference between workers adult mpo in mpo from lodes
tot_pop_lodes <- sum(workers_crop$`lodes-data-2018 Workers total conveyal.tif`, na.rm = T)
tot_pop_adult_census <- sum(tracts$pop_dec_adult)
# some what closer comparing to census households
tot_hh_census <- sum(tracts$hh_dec)
```


## Dasymetric Mapping approach
...
ggplot()+
  ...
  geom_sf(data = mpo_census_tracts, color= 'pink', fill = 'transparent')+
  geom_sf(data= communitytypes, color = "red", fill= 'transparent' )+
  theme_void()
```

```{r }
# dot density demographics

# https://tarakc02.github.io/dot-density/
  
# demo <- mpo_census_tracts %>% 
#   # st_drop_geometry() %>% 
#   select(GEOID, minority, nonminority) %>% 
#   pivot_longer(cols = c('minority', 'nonminority'), names_to = 'demo', values_to = 'pop') %>% 
#   filter(is.na(pop)==F)

#https://web.archive.org/web/20180928043346/https://www.cultureofinsight.com/blog/2018/05/02/2018-04-08-multivariate-dot-density-maps-in-r-with-sf-ggplot2/
# test <- demo %>% 
#   mutate(points = map2(.x = geom, .y = pop, ~ st_sample(.x, round(.y/100)))) %>% 
#   st_drop_geometry() %>% 
#   rename(geometry = points)
# test2 <- test %>% 
#   mutate(geometry = map(.x = geometry, ~st_coordinates(st_cast(.x, "POINT")))) 
#   st_as_sf()
#  ggplot() + 
#    geom_sf(data =tracts, fill='transparent',size = .75) + 
#    geom_sf(data= test,aes(color = demo), size=1)+ 
#    theme_void()
