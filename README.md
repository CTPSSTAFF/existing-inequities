...
Origins were identified by finding the center of every census tract in the MPO region. Destinations were selected from the essential places metric. The prepration of the origin and destinatons happened here: [`/analysis/TravelCosts_OriginDestPrep.R`](https://github.com/CTPSSTAFF/existing-inequities/blob/main/analysis/TravelCosts_OriginDestPrep.R) Travel times and trip paths were calucated for driving and transit modes using the same settings as the AM Peak access runs. Then we priced travel based on travel time weights described in the VTPI study using a median regional annual wage ($56,507.64 per worker) for the MPO region. We compared drive costs and average transit costs to report cost deltas. For more detail on the process of pricing travel see here: [`/analysis/TravelCosts_OriginDestProcess.R`](https://github.com/CTPSSTAFF/existing-inequities/blob/main/analysis/TravelCosts_OriginDestProcess.R). Full results for cost deltas are avaialbe [as a .csv here](https://github.com/CTPSSTAFF/existing-inequities/blob/main/output/travel_costs.csv) and [as geospatial lines here](https://github.com/CTPSSTAFF/existing-inequities/blob/main/output/TravelCosts.gpkg).  Note, cost deltas can only be calculated where there is both a drive cost and a transit cost. There is no cost delta where no transit route is available. We then selected a five destinations to visualize and compare in the application and report. This work happens here: [`/analysis/TravelCosts_OriginDestVisualize.R`](https://github.com/CTPSSTAFF/existing-inequities/blob/main/analysis/TravelCosts_OriginDestVisualize.R).
